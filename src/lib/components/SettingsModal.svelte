<script>
  ...
  async function onImportChange(event) {
    ...
    if (!file) return;
    const ok = confirm("Nhập JSON sẽ ghi đè dữ liệu hiện tại trên phòng Firebase này. Tiếp tục?");
    if (!ok) return;
    try {
  ...
  async function resetAll() {
    const ok = confirm("Đặt lại toàn bộ dữ liệu Lingo trên phòng Firebase này?");
    if (!ok) return;
    try {
    ...
    <div class="max-h-[78vh] overflow-y-auto px-4 py-4 sm:px-5">
      <div class="rounded-2xl border border-white/70 bg-white/65 p-4">
        ...
        <p class="mt-2 text-xs text-[color:var(--ink2)]">
          Dữ liệu được lưu trực tiếp trên Firebase Realtime Database của phòng hiện tại.
        </p>
      </div>
